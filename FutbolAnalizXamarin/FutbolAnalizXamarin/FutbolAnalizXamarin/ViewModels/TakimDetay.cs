...
namespace FutbolAnalizXamarin.ViewModels
{
    public class TakimDetay
    {
        ...
        public TakimDetay TakimDetayAl(int takim_id, string url)
        {
            using (var client = new HttpClient())
            {
                var response = client.GetAsync(url + takim_id).Result;

                if (response != null)
                {
                    var responseContent = response.Content;
                    string responseString = responseContent.ReadAsStringAsync().Result;
                    var posts = JsonConvert.DeserializeObject<TakimDetay>(responseString);
                    return posts;
                }
                return null;
            }
